...
class OrderManager:
    ...
    # ─── COPY-TRADE EXECUTOR (2026-04-23) ───────────────────────────────
    # Lee Positions creadas por el user via Telegram /yes, ejecuta FAK
    # al best bid/ask +/- 1c, y actualiza la Position a filled o closed.
    # Única vía on-chain para copy-trade whale_consensus.
    def drain_pending_fills(self):
        ...
        try:
            url = "%s/api/apps/%s/entities/Position?status=open&strategy=whale_consensus&limit=50" % (base, app_id)
            ...
            logger.warning("drain_pending_fills list fallo: %s", exc)
            return 0
        ...
        for pos_id, pos in pending:
            ...
            entry_hint = float(pos.get("entry_price") or 0.0)
            market_label = (pos.get("question") or pos.get("market") or "?")[:60]
            if not token_id or size_usdc <= 0:
                continue
            try:
                book = self.client.get_order_book(token_id)
                if side_str == "BUY":
                    best = float(book.asks[0].price) if book and book.asks else entry_hint
                    limit_price = self._round_price(best + 0.01)
                else:
                    best = float(book.bids[0].price) if book and book.bids else entry_hint
                    limit_price = self._round_price(max(0.01, best - 0.01))
                size_shares = self._round_size(size_usdc / max(0.01, limit_price))
                if size_shares < 5.0:
                    self._patch_position(pos_id, {
                        "status": "closed",
                        "pending_fill": False,
                        "close_reason": "size_too_small",
                        "close_time": _rq.utils.formatdate(usegmt=True),
                    })
                    processed += 1
                    continue
                side_const = BUY if side_str == "BUY" else SELL
                args = OrderArgs(token_id=token_id, price=limit_price,
                                 size=size_shares, side=side_const)
                log_decision(reason="copy_fill_placing", market=market_label,
                             strategy="whale_consensus", size=size_usdc,
                             extra={"pos_id": pos_id, "price": limit_price,
                                    "shares": size_shares})
                signed = self.client.create_order(args)
                resp = self.client.post_order(signed, OrderType.FAK) or {}
                order_id = resp.get("orderID") or resp.get("orderId")
                ok = bool(order_id) and resp.get("status") != "rejected"
                if ok:
                    filled_shares = 0.0
                    try:
                        st = self.client.get_order(order_id) or {}
                        filled_shares = float(st.get("size_matched")
                                              or st.get("filled_size") or 0.0)
                    except Exception:
                        filled_shares = size_shares
                    if filled_shares > 0:
                        filled_usdc = filled_shares * limit_price
                        self._patch_position(pos_id, {
                            "pending_fill": False,
                            ...
                            "size_tokens": round(filled_shares, 4),
                        })
                        self._copy_notify(
                            "✅ <b>COPY-TRADE LLENADO</b>\n%s\n%s %.2f sh @ %.3f (~$%.2f)"
                            % (market_label, side_str, filled_shares, limit_price, filled_usdc)
                        )
                        log_decision(reason="copy_fill_ok", market=market_label,
                                     strategy="whale_consensus", size=filled_usdc,
                                     extra={"pos_id": pos_id, "order_id": order_id})
                    else:
                        self._patch_position(pos_id, {
                            "status": "closed",
                            "pending_fill": False,
                            "close_reason": "fak_no_fill",
                        })
                        self._copy_notify(
                            "⚠️ <b>Copy-trade no llenó</b>\n%s\nFAK @ %.3f sin liquidez."
                            % (market_label, limit_price)
                        )
                else:
                    reject = resp.get("error") or resp.get("status") or "unknown"
                    self._patch_position(pos_id, {
                        "status": "closed",
                        "pending_fill": False,
                        "close_reason": ("rejected:%s" % reject)[:120],
                    })
                    self._copy_notify(
                        "❌ <b>Copy-trade rechazado</b>\n%s\nMotivo: <code>%s</code>"
                        % (market_label, reject)
                    )
                processed += 1
            except Exception as exc:
                logger.error("drain_pending_fills pos=%s fallo: %s", pos_id, exc)
                self._patch_position(pos_id, {
                    "status": "closed",
                    "pending_fill": False,
                    "close_reason": ("exec_error:%s" % str(exc))[:120],
                })
                self._copy_notify(
                    "🛑 <b>Copy-trade ERROR</b>\n%s\n<code>%s</code>"
                    % (market_label, str(exc)[:180])
                )
                processed += 1
        if processed:
            logger.info("drain_pending_fills: %d posiciones procesadas", processed)
        return processed
