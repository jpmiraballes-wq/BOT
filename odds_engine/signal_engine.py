...
def _binary_side_from_question(market: PolymarketMarket, outcome_name: str) -> int | None:
    """Infer token side when Polymarket hides the real side behind binary text.

    Polymarket often stores outcomes/tokens as generic binary sides while the
    real selection lives only in question/slug. For clean 'Team A vs Team B'
    questions we map first-mentioned team to YES and second-mentioned team to NO.
    For draw markets, any Odds API 'Draw' on a question/slug containing 'draw'
    maps to the primary token side. We do not require outcomes == ['Yes','No']
    because Gamma sometimes returns sparse or non-standard outcome labels.
    """
    question = market.question or market.slug or ''
    q_norm = _norm(question)
    out_norm = _norm(outcome_name)

    if out_norm == 'draw' and 'draw' in q_norm:
        return 0
