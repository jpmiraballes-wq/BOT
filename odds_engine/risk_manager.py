...
class RiskManager:
    ...
    def odds_are_fresh(self, outcome: OddsOutcome) -> bool:
        ...
        try:
            ...
            return age <= settings.default_odds_ttl_seconds
        except Exception:
            return False
    ...
    def validate_signal_inputs(
        ...
        if settings.bot_mode not in {'OBSERVE', 'PAPER'}:
        ...
        if mapping.confidence_score < settings.min_mapping_confidence:
        ...
        if market.spread is None or market.spread > settings.max_spread:
        ...
        if self.open_exposure_usd + settings.paper_trade_usd > settings.max_total_exposure_usd:
